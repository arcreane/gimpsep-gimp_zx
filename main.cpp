...
int main() 
{  //%% choose the function you wanna use
	std::cout << "Select the image editor you want to use\(enter the corresponding number to select: 1.dilatation/erosion; 2.resizing; 3.Lighten/Darken; 4.Panorama; 5.contour Canny):" << std::endl;
	int a;
	std::cin >> a;
	if(a == 1){
		...
		int g_nStructElementSize = 3; //结构元素(内核矩阵)的尺寸
												  //载入原图
		g_srcImage = imread("KamenriderVICE.jpg");
		...
		Mat g_srcImage = imread("KamenriderVICE.jpg");
		int down_width = 300;
		int down_height = 200;
		Mat resized_down;
		//resize down
		...
		Mat srcImage = imread("KamenriderVICE.jpg");
		if (!srcImage.data) {
